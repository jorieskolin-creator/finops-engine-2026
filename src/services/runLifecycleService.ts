...
const command=(id:string,action:'complete'|'fail',quality?:AcquisitionQualityPersistence,shadowTelemetry?:ShadowTelemetryPersistence)=>request({method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,run_id:id,...(action==='fail'?{failure_code:'PIPELINE_FAILED'}:{}),...(quality?{quality,shadow_telemetry:shadowTelemetry}:{})})});
