...
const errorCodes=new Set(['RUN_NOT_FOUND','RUN_INACTIVE','INVALID_RUN_TRANSITION','DEPENDENCY_UNAVAILABLE','VERCEL_GOVERNED_DISPATCH_UNSUPPORTED']);
...
const command=(id:string,action:'complete'|'fail')=>request({method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,run_id:id,...(action==='fail'?{failure_code:'PIPELINE_FAILED'}:{})})});
export async function completeRun(id:string){let first:unknown;const delays=[250,500,1000,2000];for(let attempt=0;attempt<=delays.length;attempt++){try{const completed=await command(id,'complete');if(completed.state==='completed'&&completed.cleanup_status==='verified')return completed;}catch(error){first??=error;}const status=await getRun(id).catch(()=>null);if(status?.state==='completed'&&status.cleanup_status==='verified')return status;if(status?.state!=='active'&&status?.state!=='completed')break;if(attempt<delays.length)await sleep(delays[attempt]);}throw first instanceof Error?first:new Error('RUN_DEPENDENCY_UNAVAILABLE');}
