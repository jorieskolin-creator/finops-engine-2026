...
assert.deepEqual(legacy.routes.preflight.map(profile => profile.id), ['gpt-5.5', 'claude-sonnet-4-6']);
...
assert.equal(qwenOpenAI.routes.preflight[1].id, 'gpt-5.4-mini');
...
assert.equal(openAIAnthropic.routes.preflight[0].id, 'gpt-5.4-mini');
assert.equal(openAIAnthropic.routes.preflight[1].id, 'claude-haiku-4-5-20251001');
...
assert.match(analysis, /modelRouting\.routes\.preflight\[0\]\.id/);
