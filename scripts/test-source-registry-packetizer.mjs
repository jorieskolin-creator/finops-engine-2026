...
const structuredCsvRow=structuredCsvRegistry.chunks.find(chunk=>chunk.type==='table_row');assert.equal(structuredCsvRow.row_number,6);assert.match(structuredCsvRow.text,/complete evidence complete evidence/,'packetization must use complete analysis rows instead of clipped samples');assert.doesNotMatch(structuredCsvRegistry.chunks.find(chunk=>chunk.type==='table_profile').text,/complete evidence|clipped/,'table profiles must not consume packet budget with row values');
