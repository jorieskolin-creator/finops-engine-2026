...
export function authorizeDestination(stage, provider, model, settings = {}) {
  ...
  if (!approved.some(candidate => canonicalize(settings) === canonicalize(settingsForProfile(candidate)))) {
...
export function authorizeConfiguredDestination(stage, provider, model, settings = {}, env = process.env) {
  ...
  if (!configured || canonicalize(settings) !== canonicalize(settingsForProfile(configured))) {
