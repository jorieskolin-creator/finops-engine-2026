...
export const APPROVED_PACKET_HASH_POLICY_VERSION = 'approved_packet_json_canonical_sha256_v2';
...
export const packetRuntimeDiagnostics = (packet, serialized = JSON.stringify(packet)) => ({
  declared_packet_hash: safeRuntimeValue(packet?.packet_hash),
  recomputed_content_hash: packet && typeof packet === 'object' ? approvedPacketHash(packet) : undefined,
...
export const evaluatePacketBinding = (packet, expected = {}) => {
  const matches = {
    packet_id_match: packet?.packet_id === expected.packetId,
    packet_hash_match: packet?.packet_hash === expected.packetHash,
    content_hash_match: approvedPacketHash(packet) === expected.packetHash,
