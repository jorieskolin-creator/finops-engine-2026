...
if redis.call('EXISTS',KEYS[1])==1 then return 0 end
local v=redis.call('GET',KEYS[2]); if not v then return 2 end
redis.call('SET',KEYS[3],v,'PX',ARGV[1]); redis.call('SADD',KEYS[4],KEYS[3]); redis.call('PEXPIRE',KEYS[4],ARGV[2]); redis.call('DEL',KEYS[2]); return 1`;
const childScript=`
if redis.call('EXISTS',KEYS[1])==1 then return 0 end
if redis.call('EXISTS',KEYS[2])==1 then return 2 end
redis.call('SET',KEYS[2],ARGV[1],'PX',ARGV[2]); redis.call('SADD',KEYS[3],KEYS[2]); redis.call('PEXPIRE',KEYS[3],ARGV[3]); return 1`;
const attemptPacketScript=`
if redis.call('EXISTS',KEYS[1])==1 then return 0 end
local packet=redis.call('GET',KEYS[2]); if not packet then return 2 end
local existing=redis.call('GET',KEYS[3]); if existing then return existing==packet and 1 or 3 end
redis.call('SET',KEYS[3],packet,'PX',ARGV[1]); redis.call('SADD',KEYS[4],KEYS[3]); redis.call('PEXPIRE',KEYS[4],ARGV[2]); return 1`;
...
local packet=redis.call('GET',KEYS[3]); if not packet or packet~=ARGV[2] then return 0 end
local marker=redis.call('GET',KEYS[4]); if marker then return marker==ARGV[1] and 2 or 3 end
redis.call('SET',KEYS[4],ARGV[1],'PX',ARGV[3]); redis.call('SADD',KEYS[5],KEYS[4]); redis.call('PEXPIRE',KEYS[5],ARGV[4]); return 1`;
export class RedisExecutionPlane {
  ...
  async stagePacket({runId,packetId,body,deadline,ttlMs}) { const child=this.key(runId,'packet:inactive',packetId); return this.client.eval(stageScript,{keys:[this.key(runId,'tombstone'),child,this.key(runId,'keys')],arguments:[body,String(this.ttl(deadline,ttlMs)),String(this.ttl(deadline,Number.MAX_SAFE_INTEGER))]}); }
  async activatePacket({runId,packetId,deadline,ttlMs}) { return this.client.eval(activateScript,{keys:[this.key(runId,'tombstone'),this.key(runId,'packet:inactive',packetId),this.key(runId,'packet:active',packetId),this.key(runId,'keys')],arguments:[String(this.ttl(deadline,ttlMs)),String(this.ttl(deadline,Number.MAX_SAFE_INTEGER))]}); }
  async getActivePacket(runId,packetId){ return this.client.get(this.key(runId,'packet:active',packetId)); }
  async snapshotAttemptPacket({runId,packetId,attemptId,deadline,ttlMs}){return this.client.eval(attemptPacketScript,{keys:[this.key(runId,'tombstone'),this.key(runId,'packet:active',packetId),this.key(runId,'packet:attempt',attemptId),this.key(runId,'keys')],arguments:[String(this.ttl(deadline,ttlMs)),String(this.ttl(deadline,Number.MAX_SAFE_INTEGER))]});}
  async getAttemptPacket(runId,attemptId){return this.client.get(this.key(runId,'packet:attempt',attemptId));}
  ...
  async authorizeSend({runId,attemptId,packetBody,token,deadline,ttlMs}){return this.client.eval(authorizeScript,{keys:[this.key(runId,'tombstone'),this.key(runId,'lease',attemptId),this.key(runId,'packet:attempt',attemptId),this.key(runId,'send-authorized',attemptId),this.key(runId,'keys')],arguments:[String(token),packetBody,String(this.ttl(deadline,ttlMs)),String(this.ttl(deadline,Number.MAX_SAFE_INTEGER))]});}
