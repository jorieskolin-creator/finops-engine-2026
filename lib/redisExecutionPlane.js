...
export class RedisExecutionPlane {
  ...
  async authorizeSend({runId,attemptId,packetId,packetBody,token,deadline,ttlMs}){return this.client.eval(authorizeScript,{keys:[this.key(runId,'tombstone'),this.key(runId,'lease',attemptId),this.key(runId,'packet:active',packetId),this.key(runId,'send-authorized',attemptId),this.key(runId,'keys')],arguments:[String(token),packetBody,String(this.ttl(deadline,ttlMs)),String(this.ttl(deadline,Number.MAX_SAFE_INTEGER))]});}
