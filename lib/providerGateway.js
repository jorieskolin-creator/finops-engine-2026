...
import { APPROVED_STAGE_PACKET_VERSION, evaluatePacketBinding, packetRuntimeDiagnostics, POLICY_VERSION, validateGovernedOutput } from './governance.js';
...
export function providerHandler(provider,{repository,redis,setIntervalFn=setInterval,clearIntervalFn=clearInterval}={}){return async(req,res)=>{
  ...
  try{if(!repository||!redis)({repository,redis}=getInfrastructure());const raw=await redis.getActivePacket(b.run_id,b.packet_id);if(!raw)return res.status(404).json({error:'PACKET_UNAVAILABLE'});let packet;try{packet=JSON.parse(raw);}catch{throw Object.assign(new Error(),{code:'DEPENDENCY_UNAVAILABLE'});}
    const binding=evaluatePacketBinding(packet,{packetId:b.packet_id,packetHash:b.packet_hash,runId:b.run_id,provider,stage:b.stage});
    if(binding.code){workerOperationalLog('warn','execution_packet_binding_mismatch',{run_id:b.run_id,stage:b.stage,provider,model:packet.model,outcome_code:binding.code,...binding.matches,...packetRuntimeDiagnostics(packet,raw)});throw Object.assign(new Error(),{code:binding.code});}
    ...
    workerOperationalLog('info','execution_packet_gateway_validated',{run_id:b.run_id,packet_id:b.packet_id,stage:b.stage,provider,model:packet.model,...packetRuntimeDiagnostics(packet,raw)});
