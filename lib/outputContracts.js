export const OUTPUT_CONTRACT_IDS = Object.freeze({
  ...
  roadmapFactCheck: 'finops_roadmap_fact_check_v1',
});
...
const factCheckSchema = (classifications, sourceLocations, unsupportedSeverities) => object({
  claims: {
    type: 'array',
    minItems: 1,
    maxItems: 15,
...
const validateValue = (value, schema) => {
  if (schema.type === 'string') {
    if (typeof value !== 'string' || (schema.enum && !schema.enum.includes(value))) return false;
    return true;
  }
