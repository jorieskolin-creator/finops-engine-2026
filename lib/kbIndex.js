...
export function expectedIdsFromPathname(pathname) {
  ...
  const match = filename.match(/^([A-E]) - .+ - (?:(AP-[A-E][1-5])|([A-E][1-5])|([A-E][1-5])-AP)\b/);
...
export function validateKbMetadata(meta, pathname) {
  ...
  if (meta.domain_name !== expected.domainName) problems.push(`domain_name=${meta.domain_name} expected=${expected.domainName}`);
  if (meta.stream !== expected.stream || !STREAMS.has(meta.stream)) problems.push(`stream=${meta.stream} expected=${expected.stream}`);
  if (meta.criterion_id !== expected.criterionId) problems.push(`criterion_id=${meta.criterion_id} expected=${expected.criterionId}`);
  if (meta.capability_id !== expected.capabilityId) problems.push(`capability_id=${meta.capability_id} expected=${expected.capabilityId}`);
