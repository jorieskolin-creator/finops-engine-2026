...
The structured-data acquisition lane analyzes bounded tables and emits versioned `derived_analytical_evidence_v1` records. Only authoritative, eligible, report-eligible calculations are summarized into Evidence Lane packets; shadow records and source cell values remain withheld. These summaries may support findings when cited with their derived-evidence ID and exact summary line, but the calculations remain deterministic and the Gap Analyzer has no scoring or Quality Gate authority.

Evidence retrieval has two bounded layers. Pre-analysis retrieval deterministically expands weak domain packets from the approved Source Registry. After Evidence Check identifies a weak criterion, the `evidence_gap_analysis` Workhorse stage receives only criterion definitions and low-evidence summaries—not source chunks or remote KB bodies—and proposes semantic themes/terms under `finops_evidence_gap_query_v1`. Deterministic local matching then selects at most eight new approved chunks per pass for targeted rescan, with at most two passes. Invalid or unavailable model output falls back to deterministic lexical expansion. Effective revised packets are rebuilt after all domains settle and are used for provenance reconciliation, pre-synthesis integrity, Evidence Package/RunTrace output, and subsequent analysis.
