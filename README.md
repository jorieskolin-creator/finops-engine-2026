...
The resolution-based maturity replacement is being introduced incrementally under `ADR-002`. Its canonical capability/anti-pattern pair registry is currently `SHADOW_NOT_ACTIVE`: it is validated as normative configuration but does not alter live gauges, CRAWL/WALK/RUN, or Quality Gate decisions. The engine now derives post-reconciliation criterion resolution records and stores the versioned replacement calculation in the Phase 2 checkpoint for shadow evaluation. Unknown and verification-unresolved criteria remain `NA`; provenance-bound direct or approved-derived evidence and governed tested absence can resolve criteria. Aggregate-only shadow results are included in exported RunTrace JSON for calibration, with no criterion, pair, quote, or source detail and with `scoring_authority: false`. The target model separates corroborated and observed maturity from assessment resolution, preserves anti-pattern 1/3 and 2/3 distinctions, and keeps Assessment Sufficiency separate from roadmap actionability.
