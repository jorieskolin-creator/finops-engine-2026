...
The resolution-based maturity replacement is being introduced incrementally under `ADR-002`. Its canonical capability/anti-pattern pair registry is currently `SHADOW_NOT_ACTIVE`: it is validated as normative configuration but does not yet alter live gauges, CRAWL/WALK/RUN, or Quality Gate decisions. The target model separates corroborated and observed maturity from assessment resolution, preserves anti-pattern 1/3 and 2/3 distinctions, and keeps Assessment Sufficiency separate from roadmap actionability.
