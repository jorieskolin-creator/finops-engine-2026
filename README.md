...
Completed assessments include a versioned `acquisition_quality_snapshot_v1` under `meta.acquisition_quality`. The Master Data HTML renders the same snapshot visibly. Summary and Master Data HTML are display-only: they must not embed `finops-data`, other hidden JSON, or script payloads. Restore a saved assessment from the explicit JSON download. Older HTML files that still contain `finops-data` can be imported, but newly generated HTML cannot.
